    ...
    if structures:
        for name, struct in structures.items():
            ...
            for contour in struct['contours']:
                ...
                # Usar patient_to_voxel para transformar coordenadas correctamente
                points = patient_to_voxel(raw_points, volume_info)

                # Determinar si el contorno intersecta con el slice actual
                if plane == 'axial':
                    slice_pos = slice_idx
                    mask = np.abs(points[:, 2] - slice_pos) < 1.0  # Tolerancia aumentada
                    if np.any(mask):
                        pts = points[mask][:, [0, 1]]
                        if len(pts) >= 3:
                    ...
                    slice_pos = slice_idx
                    mask = np.abs(points[:, 1] - slice_pos) < 1.0  # Tolerancia aumentada
                    if np.any(mask):
                        pts = points[mask][:, [0, 2]]
                        if len(pts) >= 3:
                            polygon = patches.Polygon(pts, closed=True, fill=False, 
                    ...
                    slice_pos = slice_idx
                    mask = np.abs(points[:, 0] - slice_pos) < 1.0  # Tolerancia aumentada
                    if np.any(mask):
                        pts = points[mask][:, [1, 2]]
                        if len(pts) >= 3:
                            polygon = patches.Polygon(pts, closed=True, fill=False, 
